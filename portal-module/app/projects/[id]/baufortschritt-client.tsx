    ...
    <div className="grid" style={{ gap: 14 }}>
      ...
      {sel && (
        <div className="grid" style={{ gap: 12, gridTemplateColumns: "1fr 1fr", alignItems: "start" }}>
          <StatusViewer3D url={`${BP}/api/baufortschritt/${sel.id}/status.glb`} />
          <div className="panel" style={{ padding: 0, maxHeight: 460, overflowY: "auto" }}>
            <div className="spread" style={{ padding: "10px 12px" }}>
              <strong className="small">Bauteile — {sel.name}</strong>
              <span className="small muted">grün gebaut · grau nicht · orange verdeckt</span>
            </div>
            <table>
              <thead><tr>
                <th>Material</th><th style={{ width: 70 }}>OK</th><th style={{ width: 110 }}>Status</th>
                <th style={{ width: 60 }}>geb%</th><th style={{ width: 70 }}>ΔZ</th>
              </tr></thead>
              <tbody>
                {(sel.elements ?? []).map((e, i) => (
                  <tr key={i}>
                    <td>{e.material ?? e.bauteil ?? "—"}</td>
                    <td className="muted">{e.kote_ok ?? "—"}</td>
                    <td><Ampel s={e.status} /></td>
