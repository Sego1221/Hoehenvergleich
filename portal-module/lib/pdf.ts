...
// Vektor-Primitive für die Höhenprofile, Bild-Einbettung für Logo + ΔZ-Karte.
// -----------------------------------------------------------------------------
...
// Profil-Linienfarben: Soll grau-blau, Ist BM-Grün, ΔZ kräftiges Rot/Magenta.
...
export interface ProtokollDaten {
  ...
  tol: number; // Toleranz in m (für "auf Soll %" + ΔZ-Karte)
  stats: ProtokollStats;
  // ΔZ-Übersichtskarte als PNG-Bytes (vom Compute-Service /dz.png).
...
export async function makeProtocolPdf(data: ProtokollDaten): Promise<Uint8Array> {
  ...
  const text = (
    ...
    pg.drawText(str, { x, y, size, font: f, color });
  };
  ...
  // ======================================================================= //
  // ΔZ-ÜBERSICHTSKARTE (eigene Seite, gross) — wie "Übersicht – Messungen".
  // ======================================================================= //
  if (dzImg) {
    const ovPage = newPage();
    let oy = PAGE_H - MARGIN;
    text(ovPage, "ΔZ-Übersichtskarte", MARGIN, oy - 14, 18, fontBold, BM);
    oy -= 30;
    text(
      ovPage,
  ...
  // ======================================================================= //
  {
    ...
    const kacheln: { label: string; value: string; color?: RGB }[] = [
      ...
      { label: "Median ΔZ", value: cm(s.medianM) },
    ];
    const cols = 3;
    const gap = 10;
...
// ---------------------------------------------------------------------------
// Höhenprofil vektoriell zeichnen (Soll/Ist/ΔZ über Distanz).
// ---------------------------------------------------------------------------
...
function drawProfil(
  ...
  // ΔZ-Kennwert (max Betrag) als Text.
  ...
  if (dzVals.length > 0) {
    ...
    text(
      pg,
      `Länge ${num(prof.lengthM ?? dSpan, 1)} m · max |ΔZ| ${cm(maxAbs)}`,
      plot.x + 2,
      plot.y - 22,
      7.5,
      font,
